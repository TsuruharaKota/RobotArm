    ...
    class RobotArm{
        public:
            ...
            Vector3d invKinema(Vector3d pos){
                ...
                theta[2] = acos((std::pow(pos(0), 2) + std::pow(pos(1), 2) + std::pow(pos(2), 2)) - L1 * L1 - L2 * L2) / (2 * L1 * L2);
                theta[0] = atan2(pos(1), pos(0));
                theta[1] = atan2(-1 * (L2 * sin(theta[2])
                                    * sqrt_x2y2) + (L1 + L2 * cos(theta[2]) * pos(2)), 
                                    (L1 + L2 * cos(theta[2]) * sqrt_x2y2 + L2 * sin(theta[2]) * pos(2)));
